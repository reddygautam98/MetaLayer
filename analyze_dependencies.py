    ...
    for dag_file in dags_dir.glob("*.py"):
        ...
        try:
            ...
            if dag_id_match:
                ...
                external_matches = re.findall(r'external_dag_id\s*=\s*["\']([^"\']+)["\']', content)
                external_deps.extend(external_matches)
                
                # Find TriggerDagRunOperator dependencies  
                trigger_matches = re.findall(r'trigger_dag_id\s*=\s*["\']([^"\']+)["\']', content)
...
def validate_dependencies(dependency_map, dag_list):
    ...
    for dag_id, info in dependency_map.items():
        ...
        if info['internal_task_deps']:
            print(f"  🔗 Task flow: {' >> '.join(set([t[0] for t in info['internal_task_deps']] + [t[1] for t in info['internal_task_deps']]))}")
