...
def analyze_system_overview():
    ...
    structure["yaml_files"] = len(list(Path(".").rglob("*.yaml"))) + len(
        list(Path(".").rglob("*.yml"))
    )
    ...
    for dag_file in dags_dir.glob("*.py"):
        ...
        try:
            ...
            if dag_info:
                ...
                if any(
                    word in dag_info["dag_id"].lower()
                    for word in ["bronze", "silver", "gold", "medallion"]
                ):
            ...
            dag_analysis["dag_details"].append(
                {"file": dag_file.name, "error": str(e), "status": "ERROR"}
            )
    ...
    for layer, expected_dags in expected_layers.items():
        ...
        for expected_dag in expected_dags:
            ...
            if any(expected_dag in str(f) for f in alt_files):
                pipeline_analysis["medallion_architecture"][layer]["found"].append(
                    expected_dag
                )
            else:
                pipeline_analysis["medallion_architecture"][layer]["missing"].append(
                    expected_dag
                )
...
def analyze_data_files():
    ...
    data_analysis = {"source_data": {}, "total_records": 0, "data_quality": {}}
    ...
    for source_file in data_sources:
        ...
        if file_path.exists():
            ...
            data_analysis["source_data"][source_file] = {"exists": False}
...
def analyze_configurations():
    ...
    for docker_file in docker_files:
        ...
        config_analysis["docker_config"][docker_file] = {
            ...
            "size_kb": (
                round(file_path.stat().st_size / 1024, 2) if file_path.exists() else 0
            ),
        }
...
def assess_deployment_readiness():
    ...
    critical_checks = {
        ...
        "main_dags_exist": all(
            Path(f"dags/{dag}.py").exists()
            or any(dag in str(f) for f in Path("dags").glob("*.py"))
            for dag in [
                "medallion_master_orchestrator",
                "bronze_layer_production",
                "silver_layer_production",
                "gold_layer_production",
            ]
        ),
        "data_files_exist": Path("data/bronze_src/erp/erp_sales.csv").exists()
        and Path("data/bronze_src/crm/crm_customers.csv").exists(),
        "sql_files_exist": Path("include/sql").exists()
        and len(list(Path("include/sql").glob("*.sql"))) > 0,
        "requirements_valid": Path("requirements.txt").exists(),
        "utilities_exist": Path("include/utils").exists()
        and len(list(Path("include/utils").glob("*.py"))) > 0,
    }
        ...
        if not passed:
            if "docker" in check_name or "main_dags" in check_name:
                readiness["blockers"].append(
                    f"Critical: {check_name.replace('_', ' ').title()}"
                )
            else:
                readiness["warnings"].append(
                    f"Warning: {check_name.replace('_', ' ').title()}"
                )
...
def generate_recommendations(report):
    ...
    if score >= 90:
        recommendations.append(
            "🎉 System is production-ready! All critical components are functional."
        )
    elif score >= 70:
        recommendations.append(
            "⚠️ System is mostly ready but has some minor issues to address."
        )
    else:
    ...
    if dag_count > 20:
        recommendations.append(
            "📊 Consider organizing DAGs into subdirectories for better management."
        )
    ...
    if total_records > 1000000:
        recommendations.append(
            "🚀 Large dataset detected - ensure database is properly optimized for performance."
        )
...
def print_report(report):
    ...
    print(
        f"Configuration files: {overview['yaml_files']} YAML, {overview['json_files']} JSON"
    )
    ...
    for layer, info in etl.items():
        ...
        print(
            f"{layer.upper()} Layer: {found_count}/{total_count} DAGs ({'✅' if found_count == total_count else '⚠️'})"
        )
    ...
    for source, info in data_analysis["source_data"].items():
        if info.get("exists"):
            print(
                f"✅ {source}: {info.get('records', 'N/A'):,} records ({info.get('size_mb', 0)} MB)"
            )
        else:
