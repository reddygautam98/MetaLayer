...
def initialize_pipeline_run(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        init_metrics = {
            ...
            "initialization_time": (
                datetime.now() - datetime.fromisoformat(pipeline_metadata["start_time"])
            ).total_seconds(),
        }
    ...
    for check in checks:
        try:
            ...
            if result and len(result) > 0:
                ...
                if check["critical"]:
                    prerequisites["failed_checks"].append(
                        {"check": check["name"], "message": "No result returned"}
                    )
                    prerequisites["all_passed"] = False
    ...
    try:
        ...
        if layer_name == "bronze":
            ...
            results = hook.get_records(
                query, parameters=[execution_date.date(), execution_date.date()]
            )

            if results:
                total_records = sum(row[0] for row in results)
                avg_quality = (
                    sum(row[1] for row in results if row[1] is not None)
                    / len([r for r in results if r[1] is not None])
                    if results
                    else 0
                )
        ...
        with hook.get_conn() as conn:
            ...
            final_summary = {
                ...
                "overall_quality_score": (
                    sum(
                        layer["metrics"]["data_quality_score"]
                        for layer in metadata.get("layers_completed", [])
                    )
                    / len(metadata.get("layers_completed", []))
                    if metadata.get("layers_completed", [])
                    else 0.0
                ),
            ...
            cursor.execute(
                ...
                [
                    ...
                    end_time,
                    final_summary["total_duration_seconds"],
                    final_summary["total_records_processed"],
                    final_summary["overall_quality_score"],
                    final_summary["sla_met"],
                    DAG_ID,
                    execution_date,
                ],
            )
