                ...
                for file_path in crm_files:
                    try:
                        ...
                        if not missing_cols:
                            ...
                            validation_results['crm_data']['issues'].append(f"Missing columns in {file_path}: {missing_cols}")
                    except Exception as e:
                        validation_results['crm_data']['issues'].append(f"Error reading {file_path}: {str(e)}")
            ...
            if erp_files:
                for file_path in erp_files:
                    try:
                        ...
                        required_cols = ['order_id', 'customer_id', 'product_id', 'order_date']
                        missing_cols = set(required_cols) - set(df.columns)
                        ...
                        if not missing_cols:
                            ...
                            validation_results['erp_data']['issues'].append(f"Missing columns in {file_path}: {missing_cols}")
                    except Exception as e:
                        validation_results['erp_data']['issues'].append(f"Error reading {file_path}: {str(e)}")
        
        # Check if any valid sources exist
        valid_sources = sum(1 for source in validation_results.values() if source['valid'])
...
def determine_processing_branch(**context) -> str:
    ...
    # Count valid sources
    valid_sources = [source for source, data in validation_results.items() if data['valid']]
...
def ingest_crm_data(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        for file_path in crm_files:
            ...
            for chunk_idx, chunk in enumerate(chunk_iter):
                try:
                    ...
                    logger.info(f"✅ Processed chunk {chunk_idx}: {len(records)} records")
                    ...
                    if metrics['errors'] > MAX_ERRORS:
                        raise DataQualityError(f"Too many errors ({metrics['errors']}) exceeded threshold")
        ...
        if metrics['records_processed'] > 0:
            metrics['data_quality_score'] = (metrics['records_inserted'] / metrics['records_processed'])
        ...
        if metrics['data_quality_score'] < QUALITY_THRESHOLD:
            raise DataQualityError(f"Data quality score {metrics['data_quality_score']} below threshold {QUALITY_THRESHOLD}")
    ...
    try:
        ...
        for file_path in erp_files:
            ...
            for chunk_idx, chunk in enumerate(chunk_iter):
                try:
                    # Data cleaning and standardization
                    chunk = chunk.dropna(subset=['order_id', 'customer_id', 'product_id'])
                    ...
                    hook.insert_rows(
                        ...
                        target_fields=[
                            ...
                            'quantity', 'price', 'total_amount', 'source_system',
                            'ingestion_timestamp', 'file_name', 'chunk_id'
                        ],
                        replace=True
                    )
...
def log_pipeline_completion(**context) -> None:
    ...
    try:
        ...
        crm_metrics = context['task_instance'].xcom_pull(task_ids='ingest_crm_data') or {}
        erp_metrics = context['task_instance'].xcom_pull(task_ids='ingest_erp_data') or {}
        validation_results = context['task_instance'].xcom_pull(task_ids='validate_bronze_layer') or {}
        
        # Aggregate metrics
        total_records = crm_metrics.get('records_inserted', 0) + erp_metrics.get('records_inserted', 0)
        ...
        hook.insert_rows(
            ...
            target_fields=[
                'dag_id', 'task_id', 'execution_date', 'start_time', 
                'end_time', 'status', 'records_processed', 'error_message'
            ]
        )
...
single_source_task = DummyOperator(
    task_id='single_source_ingestion',
    dag=dag
)
