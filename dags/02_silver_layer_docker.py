...
MAX_PROCESSING_TIME = int(Variable.get("silver_max_processing_minutes", default_var=120))
...
def transform_customers_bronze_to_silver(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        with hook.get_conn() as conn:
            ...
            while True:
                ...
                columns = [
                    ...
                    'address', 'created_date', 'source_system', 'ingestion_timestamp'
                ]
                ...
                try:
                    ...
                    silver_columns = [
                        'customer_id', 'customer_name_clean', 'email', 'phone_clean',
                        'address', 'created_date_clean', 'is_active', 'data_quality_score',
                        'processed_timestamp'
                    ]
                    
                    df_final = df_transformed[
                        df_transformed['data_quality_score'] >= 0.3  # Minimum quality threshold
                    ][silver_columns].copy()
                    
                    # Rename columns for Silver schema
                    df_final.columns = [
                        'customer_id', 'customer_name', 'email', 'phone',
                        'address', 'created_date', 'is_active', 'data_quality_score',
                        'processed_timestamp'
                    ]
                    ...
                    if not df_final.empty:
                        ...
                        records = [tuple(row) for row in df_final.itertuples(index=False)]
                    ...
                    if metrics['transformation_errors'] > 10:  # Max allowed errors
                        raise DataTransformationError(f"Too many transformation errors: {metrics['transformation_errors']}")
        ...
        if metrics['records_transformed'] > 0:
            metrics['data_quality_score'] = metrics['records_loaded'] / metrics['records_transformed']
        ...
        if metrics['data_quality_score'] < QUALITY_THRESHOLD:
            logger.warning(f"⚠️ Quality score {metrics['data_quality_score']:.2%} below threshold {QUALITY_THRESHOLD:.2%}")
        ...
        with hook.get_conn() as conn:
            ...
            while True:
                ...
                try:
                    ...
                    # Prepare for Silver layer insert
                    silver_columns = [
                        'order_id', 'customer_id', 'product_id', 'order_date_clean',
                        'quantity_clean', 'unit_price_clean', 'total_amount_clean',
                        'is_valid', 'processed_timestamp'
                    ]
                    
                    df_final = df_transformed[
                        df_transformed['data_quality_score'] >= 0.5  # Quality threshold for orders
                    ][silver_columns].copy()
...
def validate_silver_layer_quality(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        validation_results['customer_quality_checks'] = validator.run_validation_checks(customer_checks)
        ...
        validation_results['order_quality_checks'] = validator.run_validation_checks(order_checks)
        ...
        validation_results['cross_table_checks'] = validator.run_validation_checks(cross_checks)
        ...
        for check_result in all_checks:
            if not check_result.get('passed', False):
                validation_results['critical_issues'].append(check_result.get('message', 'Unknown issue'))
        
        logger.info(f"✅ Silver layer validation completed: Quality Score = {validation_results['overall_quality_score']:.2%}")
        
        if validation_results['critical_issues']:
            logger.warning(f"⚠️ Critical issues found: {validation_results['critical_issues']}")
...
def create_silver_summary_stats(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        customer_stats = hook.get_first(customer_stats_query, parameters=[execution_date.date()])
        if customer_stats:
            summary_stats['customers'] = dict(zip([
                'total_customers', 'active_customers', 'avg_quality_score', 
                'first_processed', 'last_processed'
            ], customer_stats))
        ...
        order_stats = hook.get_first(order_stats_query, parameters=[execution_date.date()])
        if order_stats:
            summary_stats['orders'] = dict(zip([
                'total_orders', 'valid_orders', 'total_revenue', 
        ...
        hook.insert_rows(
            ...
            rows=[(
                ...
                summary_stats.get('orders', {}).get('total_orders', 0),  # total_duration_seconds (placeholder)
                summary_stats.get('orders', {}).get('avg_order_value', 0),  # records_per_second (placeholder) 
                0,  # memory_usage_mb (placeholder)
                0   # cpu_usage_percent (placeholder)
            )],
            target_fields=[
                'dag_id', 'execution_date', 'total_duration_seconds',
                'records_per_second', 'memory_usage_mb', 'cpu_usage_percent'
            ]
        )
