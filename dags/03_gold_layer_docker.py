    ...
    try:
        ...
        with hook.get_conn() as conn:
            ...
            columns = [
                ...
                'address', 'is_active', 'data_quality_score', 'processed_timestamp'
            ]
            ...
            for _, silver_row in df_silver.iterrows():
                ...
                if current_record.empty:
                    ...
                    cursor.execute(insert_query, [
                        customer_id, silver_row['customer_name'], silver_row['email'],
                        silver_row['phone'], silver_row['address'], silver_row['is_active'],
                        silver_row['data_quality_score'], execution_date,
                        datetime(9999, 12, 31), True
                    ])
                    ...
                    changes_detected = (
                        current['customer_name'] != silver_row['customer_name'] or
                        current['email'] != silver_row['email'] or
                        current['phone'] != silver_row['phone'] or
                        current['address'] != silver_row['address'] or
                        current['is_active'] != silver_row['is_active']
                    )
                    ...
                    if changes_detected:
                        ...
                        """
                        cursor.execute(update_query, [execution_date, current['customer_key']])
...
def build_order_fact_table(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        with hook.get_conn() as conn:
            ...
            cursor.execute(monthly_agg_query, [
            ...
            for category, metrics in kpi_results.items():
                for metric_name, metric_value in metrics.items():
                    if isinstance(metric_value, (int, float)):
                        cursor.execute(kpi_insert_query, [
                            date_key, f"{category}_{metric_name}", metric_value, str(metrics)
                        ])
...
def validate_gold_layer_quality(**context) -> Dict[str, Any]:
    ...
    try:
        ...
        validation_results['dimension_checks'] = validator.run_validation_checks(dimension_checks)
        ...
        logger.info(f"✅ Gold layer validation completed: Quality Score = {validation_results['overall_quality_score']:.2%}")
    ...
    try:
        ...
        with hook.get_conn() as conn:
            ...
            if customer_data:
                summary['customer_insights'] = {
                    ...
                    'avg_quality_score': float(customer_data[1]) if customer_data[1] else 0.0
                }
        ...
        hook.insert_rows(
            ...
            rows=[(
                ...
                summary.get('customer_insights', {}).get('total_active_customers', 0)
            )],
            target_fields=['execution_date', 'total_orders', 'total_revenue', 'active_customers']
        )
...
build_order_facts_task = PythonOperator(
    task_id='build_order_fact_table',
    python_callable=build_order_fact_table,
    dag=dag
)

# Calculate business KPIs
calculate_kpis_task = PythonOperator(
    task_id='calculate_business_kpis',
    python_callable=build_business_kpis,
    dag=dag
)
