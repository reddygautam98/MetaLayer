...
def load_csv_to_table(csv_path: str, table: str):
    ...
    with engine.begin() as conn:
        ...
        df.to_sql(name=table.split(".")[-1], schema=table.split(".")[0], con=conn, if_exists="append", index=False)

def load_erp_orders():
    load_csv_to_table(os.path.join(DATA_DIR, "erp", "orders.csv"), "bronze.erp_orders_raw")

def load_crm_customers():
    load_csv_to_table(os.path.join(DATA_DIR, "crm", "customers.csv"), "bronze.crm_customers_raw")
...
with DAG(
    ...
    load_crm = PythonOperator(task_id="load_crm_customers", python_callable=load_crm_customers)
