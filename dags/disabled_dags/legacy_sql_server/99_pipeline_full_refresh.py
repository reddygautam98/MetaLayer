...
def load_table(csv_rel_path, target):
    ...
    with engine.begin() as conn:
        ...
        df.to_sql(name=target.split(".")[-1], schema=target.split(".")[0], con=conn, if_exists="append", index=False)
...
with DAG(
    ...
    bronze_crm = PythonOperator(
        task_id="bronze_crm",
        python_callable=lambda: load_table("crm/customers.csv", "bronze.crm_customers_raw")
    )
