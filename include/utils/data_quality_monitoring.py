...
try:
    ...
    logging.warning("Great Expectations not available. Install with: pip install great_expectations")
    GX_AVAILABLE = False
...
class DataQualityMonitor:
    ...
    def __init__(self, 
        ...
        if GX_AVAILABLE:
            ...
            logger.warning("Great Expectations not available - using basic quality checks")
    ...
    def _initialize_great_expectations(self):
        ...
        try:
                ...
                stores={
                    "expectations_store": {
                        ...
                        "store_backend": {
                            ...
                            "base_directory": str(self.context_root_dir / "expectations")
                        }
                    },
                    "validations_store": {
                        ...
                        "store_backend": {
                            "class_name": "TupleFilesystemStoreBackend", 
                            "base_directory": str(self.context_root_dir / "validations")
                        }
                    },
                    ...
                    "checkpoint_store": {
                        ...
                        "store_backend": {
                            "class_name": "TupleFilesystemStoreBackend",
                            "base_directory": str(self.context_root_dir / "checkpoints")
                        }
                    }
                },
            ...
            # Initialize context
            self.context = gx.data_context.BaseDataContext(project_config=data_context_config)
    ...
    def _ensure_monitoring_tables(self):
        ...
        try:
            self.hook.run([create_schema_sql, create_metrics_table, create_results_table])
    ...
    def create_expectation_suite(self, 
        ...
        try:
            ...
            batch_request = RuntimeBatchRequest(
                ...
                runtime_parameters={"query": f"SELECT * FROM {schema_name}.{table_name} LIMIT 10000"},
                batch_identifiers={"default_identifier_name": f"{schema_name}_{table_name}"}
            )
                ...
                if pd.api.types.is_numeric_dtype(column_data):
                    ...
                    if pd.notna(min_date) and pd.notna(max_date):
                        validator.expect_column_values_to_be_between(
                            column,
                            min_value=min_date,
                            max_value=datetime.now() + timedelta(days=365),  # Allow future dates up to 1 year
                            mostly=0.99
                        )
            ...
            suite_info = {
                ...
                'total_expectations': len(validator.get_expectation_suite().expectations),
                'columns_analyzed': len(columns),
                'created_at': datetime.now().isoformat()
            }
            
            logger.info(f"Created expectation suite for {schema_name}.{table_name}: {suite_info}")
            return suite_info
    ...
    def run_quality_check(self, 
                         ...
                         expectation_suite_name: Optional[str] = None) -> QualityCheckResult:
    ...
    def _run_great_expectations_check(self, 
                                    table_name: str, 
                                    schema_name: str,
                                    expectation_suite_name: str,
                                    check_id: str) -> QualityCheckResult:
        ...
        try:
            ...
            for expectation_result in validation_result.results:
                metric = DataQualityMetric(
                    ...
                    table_name=f"{schema_name}.{table_name}",
                    column_name=expectation_result.expectation_config.kwargs.get('column'),
    ...
    def _run_basic_quality_check(self, 
                                table_name: str, 
                                schema_name: str,
                                check_id: str) -> QualityCheckResult:
        ...
        try:
            ...
            # Basic checks
            checks = [
                ("row_count", f"SELECT COUNT(*) FROM {full_table_name}", lambda x: x > 0),
                ("null_check", f"SELECT COUNT(*) FROM {full_table_name} WHERE {self._get_primary_key(schema_name, table_name)} IS NULL", lambda x: x == 0),
                ("duplicate_check", f"SELECT COUNT(*) - COUNT(DISTINCT {self._get_primary_key(schema_name, table_name)}) FROM {full_table_name}", lambda x: x == 0),
            ]
    ...
    def _store_quality_results(self, result: QualityCheckResult):
        ...
        try:
            ...
            self.hook.run(result_sql, parameters=(
                result.check_id, result.table_name, result.total_expectations,
                result.successful_expectations, result.failed_expectations,
                result.success_rate, result.execution_time,
                json.dumps(result.summary), result.timestamp
            ))
            ...
            for metric in result.metrics:
                ...
                self.hook.run(metric_sql, parameters=(
                    result.check_id, metric.metric_name, metric.table_name,
                    metric.column_name, metric.metric_value, metric.threshold_value,
                    metric.status, json.dumps(metric.details) if metric.details else None,
                    metric.timestamp
                ))
    ...
    def get_quality_dashboard_data(self, 
        ...
        try:
            ...
            table_summary = self.hook.get_records(table_summary_query, parameters=(cutoff_date,))
            ...
            failed_metrics = self.hook.get_records(failed_metrics_query, parameters=(cutoff_date,))
            ...
            return {
                ...
                'summary': {
                    ...
                    'avg_quality_score': sum(row[1] for row in table_summary if row[1]) / len(table_summary) if table_summary else 0,
                    'tables_below_threshold': len([row for row in table_summary if row[1] and row[1] < 95]),
...
def run_table_quality_check(table_name: str, 
                           schema_name: str = 'bronze',
                           conn_id: str = 'postgres_default') -> Dict[str, Any]:
