...
class IncrementalProcessor:
    ...
    def update_state(self, state: IncrementalState):
        ...
        try:
            ...
            self.hook.run(upsert_sql, parameters=(
                state.table_name, state.strategy, state.last_processed_timestamp,
                state.last_processed_hash, state.last_processed_record_id,
                state.processed_record_count, state.last_run_success,
                state.error_count, state.created_at, state.updated_at
            ))
        ...
        try:
            ...
            for chunk_df in pd.read_csv(csv_path, chunksize=self.chunk_size):
                ...
                if timestamp_column in chunk_df.columns:
                    ...
                    if last_timestamp:
                        new_chunk = chunk_df[chunk_df[timestamp_column] > last_timestamp]
                    else:
                        new_chunk = chunk_df
            ...
            result = {
                ...
                'last_processed_timestamp': last_timestamp.isoformat() if last_timestamp else None
            }
...
def create_incremental_processor(conn_id: str = 'postgres_default') -> IncrementalProcessor:
