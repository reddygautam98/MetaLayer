...
data_quality_score = Gauge(
    "metalayer_data_quality_score", "Data quality score from 0 to 1", ["layer", "table"]
)
...
def simulate_database_metrics():
    ...
    while True:
        try:
            ...
            db_pool_active_connections.labels(pool_name="postgres_main").set(
                active_connections
            )
            db_pool_total_connections.labels(pool_name="postgres_main").set(
                total_pool_size
            )

            logger.info(
                f"Updated DB metrics - Active: {active_connections}/{total_pool_size}"
            )
        ...
        try:
            for layer in layers:
                for table in tables:
                    ...
                    data_quality_score.labels(layer=layer, table=table).set(
                        quality_score
                    )
                ...
                for table in tables:
                    for operation in operations:
                        ...
                        incremental_processing_duration.labels(
                            layer=layer, table=table, operation=operation
                        ).observe(duration)
...
def export_pipeline_metrics(
    dag_id, task_id, status, execution_time=None, records_processed=0
):
    """
