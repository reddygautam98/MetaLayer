...
def create_connections() -> int:
    ...
    for conn in AIRFLOW_CONNECTIONS:
        ...
        if conn['conn_type'] == 'postgres':
            uri = (f"postgresql://{conn['login']}:{conn['password']}"
        ...
        command = [
            'connections', 'add', conn['conn_id'],
            '--conn-type', conn['conn_type'],
            '--conn-host', conn['host'],
            '--conn-port', str(conn['port']),
            '--conn-login', conn.get('login', ''),
            '--conn-password', conn.get('password', ''),
            '--conn-schema', conn.get('schema', ''),
...
def verify_configuration() -> Dict[str, Any]:
    ...
    verification_results['verification_success'] = (
        verification_results['connections_verified'] == verification_results['total_connections'] and
        verification_results['variables_verified'] == verification_results['total_variables']
    )
    
    logger.info(f"📋 Verification Results:")
    logger.info(f"   Connections: {verification_results['connections_verified']}/{verification_results['total_connections']}")
    logger.info(f"   Variables: {verification_results['variables_verified']}/{verification_results['total_variables']}")
...
def initialize_database_schema() -> bool:
    ...
    sql_file_path = Path(__file__).parent / "include" / "sql" / "complete_schema_docker.sql"
    ...
    try:
        ...
        command = [
            'docker', 'exec', '-i', 'metalayer-postgres-1',
            'psql', '-U', POSTGRES_CONFIG['username'], 
...
def main():
    ...
    if not check_airflow_availability():
        logger.error("❌ Airflow CLI not available. Please ensure Airflow is running and accessible.")
        sys.exit(1)
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info(f"Database Schema: {'✅ Success' if schema_success else '❌ Failed'}")
        logger.info(f"Connections Created: {connections_created}/{len(AIRFLOW_CONNECTIONS)}")
        logger.info(f"Variables Created: {variables_created}/{len(AIRFLOW_VARIABLES)}")
        logger.info(f"Overall Success: {'✅ Yes' if verification_results['verification_success'] and schema_success else '❌ No'}")
        ...
        if verification_results['verification_success'] and schema_success:
            ...
            logger.error("📝 Please check the logs and fix any issues before running the pipeline.")
            sys.exit(1)
