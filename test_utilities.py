...
def test_data_quality_fallback():
    ...
    try:
        ...
        if all(param in params for param in expected_params):
            ...
            results.append(f"❌ basic_data_quality_check - Incorrect signature. Expected: {expected_params}, Got: {params}")
...
def test_connection_pooling():
    ...
    try:
        ...
        import inspect
        methods = [name for name, method in inspect.getmembers(DatabaseConnectionPool, inspect.isfunction)]
        
        expected_methods = ['get_connection', 'close_all_connections']
        found_methods = [m for m in expected_methods if m in methods or f'_DatabaseConnectionPool__{m}' in dir(DatabaseConnectionPool)]
        
        if len(found_methods) >= len(expected_methods) // 2:  # Allow for some flexibility
            results.append("✅ connection_pooling - Expected methods available")
        else:
            results.append(f"⚠️  connection_pooling - Some methods missing: {expected_methods}")
        ...
        for dashboard_file in dashboard_files:
            try:
                ...
                if all(field in dashboard_data for field in required_fields):
                    ...
                    results.append(f"⚠️  Dashboard - {dashboard_file.name} (Missing required fields)")
                    
            except json.JSONDecodeError as e:
                results.append(f"❌ Dashboard - {dashboard_file.name} (Invalid JSON: {e})")
            except Exception as e:
