...
class TestDataQuality:
    ...
    def test_bronze_layer_completeness(self, db_connection):
        ...
        for table in tables:
            try:
                ...
                assert null_timestamps == 0, f"Found {null_timestamps} records with null timestamps in {table}"
    ...
    def test_silver_layer_data_quality(self, db_connection):
        ...
        try:
            ...
            assert invalid_emails == 0, f"Found {invalid_emails} invalid email addresses"
    ...
    def test_gold_layer_integrity(self, db_connection):
        ...
        try:
            ...
            assert orphaned_records == 0, f"Found {orphaned_records} orphaned fact records"
            ...
            assert invalid_amounts == 0, f"Found {invalid_amounts} invalid amounts in fact table"
    ...
    def test_data_freshness(self, db_connection):
        ...
        for table in tables_to_check:
            try:
                ...
                if result:
                    assert result > freshness_threshold, f"Data in {table} is stale (older than 24 hours)"
    ...
    def test_duplicate_detection(self, db_connection):
        ...
        for table, key_column in duplicate_checks.items():
            try:
                ...
                assert duplicate is None, f"Found duplicate {key_column} in {table}: {duplicate[0] if duplicate else 'None'}"
    ...
    def test_business_rules_validation(self, db_connection):
        ...
        try:
            ...
            if total_sales > 0:
                ...
                assert large_sales_percentage < 5, f"Too many unusually large sales: {large_sales_percentage:.2f}%"
            ...
            try:
                ...
                if total_records > 0:
                    for column in columns:
                        ...
                        assert null_percentage <= null_threshold, \
