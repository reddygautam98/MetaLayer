        ...
        try:
            with open(sql_file, "r") as f:
                ...
                if len(content.strip()) > 0:
                    ...
                    print(
                        f"✅ {sql_file.name}: {parse_time:.4f}s ({len(content)} chars)"
                    )
                else:
...
def generate_performance_report(dag_times, sql_times):
    ...
    report = {
        ...
        "summary": {
            ...
            "avg_dag_import_time": (
                sum(dag_times.values()) / len(dag_times) if dag_times else 0
            ),
            "avg_sql_parse_time": (
                sum(sql_times.values()) / len(sql_times) if sql_times else 0
            ),
        },
    }
...
def main():
    ...
    if report["summary"]["avg_dag_import_time"] > 0:
        print(
            f"   Avg DAG import time: {report['summary']['avg_dag_import_time']:.4f}s"
        )
